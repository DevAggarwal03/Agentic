...
app.get('/api/1inch/allowance', async (req: Request, res: Response) => {
  ...
  try {
    const response = await axios.get(
      `https://api.1inch.dev/swap/v6.0/8453/approve/allowance`,
      {
        headers: {
...
app.get('/api/1inch/approve', async (req: Request, res: Response) => {
  const { tokenAddress, amount } = req.query;
    ...
    const url = "https://api.1inch.dev/swap/v6.0/8453/approve/transaction";
    const config = {
        headers: {
    "Authorization": `Bearer ${process.env.ONEINCH_API_KEY}`
    },
...
app.get('/api/1inch/swap', async (req: Request, res: Response) => {
    ...
    const url = "https://api.1inch.dev/swap/v6.0/8453/swap";
    const config = {
        headers: {
            "Authorization": `Bearer ${process.env.ONEINCH_API_KEY}`
        },
...
app.get('/api/1inch/spender', async (req: Request, res: Response) => {
  try {
    const response = await axios.get(
      `https://api.1inch.dev/swap/v6.0/8453/approve/spender`,
      {
        headers: {
          'Authorization': `Bearer ${process.env.ONEINCH_API_KEY}`
        }
